...
```{r model1}
mod1<-glmer(abund~people+boats+pets+(1|site)+(month|year), na.action = "na.fail", family = "poisson")
summary(mod1)
```
...
```{r mcmc1}
mod2<-MCMCglmm(abund~people+boats+pets, random = ~ site, data=data, verbose = FALSE, family = "poisson")
summary(mod2)
```
...
```{r mind1}
mod3<-glmer(abund~index_1+(1|site)+(month|year), na.action = "na.fail", family = "poisson")
summary(mod3)
```
...
```{r mind2}
mod4<-glmer(abund~index_2+(1|site)+(month|year), na.action = "na.fail", family = "poisson")
summary(mod4)
```
